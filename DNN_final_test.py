...
from functions import dictionary,update_mu_Sigma_delay,update_mu_Sigma,update_mu_Sigma_MSBL,C2R,R2C,complex_matrix_multiplication,Fixed_Phi_Layer_FR,A_R_Layer_FR
...
model = SBL_net(Mr, Nr, G_polar, G_angle, sigma_2, num_sc, num_layers, num_filters, kernel_size)

batch_size = 2

best_model_path = './models/%dLayers_%dBeams_%dSNR.h5'%(num_layers,Mr,SNR)

model.load_weights(best_model_path)

H_predictions = model.predict(Y_real_imag_list,batch_size=batch_size,verbose=1)
H_predictions = R2C(H_predictions)

error_nmse = 0
for i in range(test_num):
    error_nmse = error_nmse + (np.linalg.norm(H_predictions[i] - H_list[i]) / np.linalg.norm(H_list[i])) ** 2
mean_nmse = error_nmse/test_num
print('NMSE:',mean_nmse)

file_handle=open('./results/DNN_performance',mode='a+')
file_handle.write('Mr=%d, SNR=%d dB:\n'%(Mr,SNR))
file_handle.write(str(mean_nmse))
file_handle.write('\n')
file_handle.write('\n')
file_handle.close()