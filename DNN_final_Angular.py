...
def SBL_net(Mr, Nr, G, sigma_2, num_sc, num_layers, num_filters, kernel_size):
    ...
    for i in range(num_layers):
        ...
        conv_layer1 = Conv1D(name='SBL_%d1'%i,filters=num_filters,kernel_size=kernel_size,strides=1,padding='same',activation='relu')
        conv_layer2 = Conv1D(name='SBL_%d2'%i,filters=1,kernel_size=kernel_size,strides=1,padding='same',activation='relu')

        temp = conv_layer1(temp) # (?,num_sc,G,num_filters)

        # average 
        temp = tf.reduce_mean(temp,axis=1,keepdims=False) # (?,G,num_filters)
 
        temp = conv_layer2(temp) # (?,G,1)
        
        alpha_hat_list = tf.tile(tf.expand_dims(temp,axis=1),(1,num_sc,1,1)) # (?,num_sc,G,1)
...
model_list = SBL_net(Mr, Nr, G_angle, sigma_2, num_sc, num_layers, num_filters, kernel_size)

epochs = 200 # 1000/200
batch_size = 16

# weight initialization
init_weights_R = C2R(A_list)
init_weights_Phi = C2R(Phi_list)
...
for model in model_list[model_count_start:]:
    ...
    # Training
    # define callbacks
    best_model_path = './models/%dLayers_%dBeams_%dSNR_Angular_Dict.h5'%(model_count_start+model_count,Mr,SNR)
